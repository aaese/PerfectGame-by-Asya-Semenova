...
int main(int argc, const char* argv[])
{
	...
	player_sprite.setTextureRect(IntRect(0, 0, 51, 90));
	...
	while (1)
	{
		...
		while (window.pollEvent(event))
		{
			if (event.type == Event::KeyPressed)
			{
				...
				{	
					...
					player_sprite.setTextureRect(IntRect(51, 0, -51, 90));
					pos = my_player->getPos();
					while (state._map[pos.second + 1][pos.first] == GameState::Block::Background) {
		...
		player_sprite.setPosition(my_player->getPos().first * 90 + ((90 - 51) / 2), my_player->getPos().second * 90);
		...
		other_player.setTextureRect(IntRect(0, 0, 51, 90));
		...
		for (auto& it : state._players) 
		{
			if (it.first != name) 
			{
				...
				other_player.setPosition(it.second.getPos().first * 90+((90-51)/2), it.second.getPos().second * 90);
